...
/**
 * Governed CSV / XLSX export buttons for the backup history dataset.
 *
 ...
 * surfaces server errors gracefully. `params` are the active filters only — never
 * `reason`/`format`.
 */
export function ExportControls({
  ...
  const runDownload = async (format: Format, withReason: string, viaModal: boolean) => {
    ...
    try {
      ...
      const message = err instanceof ApiError ? err.message : "Failed to export backup history";
      ...
      <Modal title="Export — reason required" open={pending !== null} onClose={() => setPending(null)}>
        <div className="space-y-4">
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-400">
            No start date is set — this is a broad, all-time export and requires a reason for the
            audit trail.
          </div>
          <Field
